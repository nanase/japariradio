...
void ssd1306_fill(SSD1306_COLOR color) {
  for (uint32_t i = 0; i < sizeof(ssd1306_buffer); i++) ssd1306_buffer[i] = (color == ssd1306_black) ? 0x00 : 0xFF;
}
...
void ssd1306_updateScreen(void) {
  for (uint8_t i = 0; i < 8; i++) {
    ...
    HAL_I2C_Mem_Write(&SSD1306_I2C_PORT, SSD1306_I2C_ADDR, SSD1306_SETSTARTLINE, 1, &ssd1306_buffer[SSD1306_WIDTH * i],
                      SSD1306_WIDTH, 100);
  }
}
